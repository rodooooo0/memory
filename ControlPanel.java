...
public class ControlPanel extends Frame 
{
  ...
  public void init( Kernel useKernel , String commands , String config ) 
  {
    ...
    segmentLabel.reshape(395, 240+25, 150, 15);
    segmentLabel.setFont(new Font("Courier", Font.BOLD, 14));
    add(segmentLabel);

    // Nuevo: Colores para los botones según segmento
    Color[] segmentColors = {
    ...
    // Para los primeros 32 botones (ajusta si usas más páginas)
    for (int i=0; i<32; i++) {
      int seg = 0;
      for(int s=0;s<Kernel.SEGMENTS.length;s++) {
        for(int p=0;p<Kernel.SEGMENTS[s].length;p++) {
          if(Kernel.SEGMENTS[s][p]==i) seg = s;
        }
      }
